...
import { snaptradeRequest, decrypt, SNAPTRADE_PERSONAL_SECRET_SENTINEL } from "./snaptrade.ts";
import { listTradovateAccounts, listTradovateExecutions, getTradovateContracts, decryptTradovateCredentials } from "./tradovate.ts";
import { rithmicConnect, rithmicListAccounts, rithmicReplayExecutions, rithmicDisconnect, decryptRithmicCredentials } from "./rithmic.ts";
...
// Pulls one user's connected broker accounts (SnapTrade / Tradovate / Rithmic)
// + recent activity, and upserts them into brokerage_accounts / trades.
// Idempotent via the trades(user_id, external_id) unique index. Shared by
// the per-user broker-sync edge function and the cron-broker-sync-all job.
export async function syncUserBrokers(admin: ReturnType<typeof createClient>, userId: string): Promise<{ synced: number; added: number }> {
  ...
  if (!st && !tradovate && !rithmic) return { synced: 0, added: 0 };
