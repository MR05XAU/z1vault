...
// Pulls this user's SnapTrade accounts + recent buy/sell activities and
// upserts them into brokerage_accounts / trades. Idempotent via the
// trades(user_id, external_id) unique index — safe to call repeatedly.
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: st } = await admin
      ...
      .eq("user_id", userId)
      .maybeSingle();
    if (!st) return json({ error: "not_registered" }, 400);
    ...
    try {
      ...
      return json({ synced: accounts.length, added });
    } catch (err) {
